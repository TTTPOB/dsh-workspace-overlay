 ...
 *
 * Ported from the rc.6 `@deepseek-ai/dsh-mcp-client` `connection.ts` (MIT,
 * Copyright (c) 2026 DeepSeek — see the repository README for attribution),
 * verified against the installed rc.6 `lib/types/connection.d.ts`.
 *
 * @module dsh-workspace-overlay/mcp/connection
 */
...
export type { ConnectionHandle, ConnectionOutcome, GenerationNotification, ReconnectConfig, ResolvedReconnectPolicy } from './types.js'
 ...
 * @param onGeneration - Optional generation-change observer for the future
 *   workspace manager; receives every committed swap/rollback/give-up/dispose
 *   change this supervisor or its tool syncs emit. Extension over the official
 *   rc.6 signature, which takes no observer.
...
export function startConnection(
  ...
  const opts: ToolBridgeOptions = {
    ...
    ...onGeneration === undefined ? {} : { onGeneration },
  }
  ...
  function scheduleReconnect(): void {
    ...
    if (failedAttempts > policy.maxAttempts) {
      ...
      syncChain = syncChain.then(() => {
        ...
        disposers = new Map()
        opts.onGeneration?.({ serverName: config.serverName, names: [], status: 'unregistered' })
      })
