...
import type { McpResult, ToolBridgeOptions, ToolDisposers } from './types.js'

export type { McpResult, ToolBridgeOptions, ToolDisposers } from './types.js'
export type { GenerationNotification } from './types.js'
 ...
 * same public name.
 *
 * Known-answer values below are derived from the installed rc.6 bundle and
 * pinned in the tests.
 *
...
export async function syncTools(
  ...
  try {
    ...
    opts.onGeneration?.({ serverName: opts.serverName, names: [], status: 'unregistered' })
    if (opts.registrationFailure === 'throw') throw error
    return new Map()
  }
  opts.onGeneration?.({ serverName: opts.serverName, names: [...disposers.keys()], status: 'registered' })
  return disposers
}
