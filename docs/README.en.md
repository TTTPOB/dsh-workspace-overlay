...
- **MCP interaction**: a workspace MCP row is replaced wholesale on reload — the old process exits, a fresh one starts with the workspace's own cwd/env, and the mask over the global same-named namespace is rebuilt on the serial commit chain (the replacement restriction is installed before the old mask is removed; there is no window with the mask fully lifted); a broken MCP startup fails the reload without killing the Agent/workspace, and fixing the file recovers on the next event.
...
- No draining of arbitrary in-flight third-party tool executions.
