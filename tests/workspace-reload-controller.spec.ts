 ...
 * its creation arguments and lets tests emit `add`/`change`/`unlink`/`error`
 * events by hand, and the timer is a manual fake whose callbacks tests fire
 * explicitly. Reload passes are deferred promises the tests resolve/reject,
 * so timing is fully controlled.
 */
...
describe('WorkspaceReloadController', () => {
  describe('construction', () => {
    it('creates one watcher for the exact path with ignoreInitial and starts idle', () => {
      const controller = createController()

      expect(factory.calls).toEqual([{ path: CONFIG_PATH, options: { ignoreInitial: true } }])
      expect(factory.watchers).toHaveLength(1)
      // Watching is true immediately, before any watcher "ready" state; the
      // fake never emits ready.
    ...
    it('accepts a zero debounce window', () => {
      const controller = createController({ debounceMs: 0 })
    ...
    it('propagates a throwing watch factory without leaking anything', () => {
      ...
      expect(() => createController()).toThrow(boom)
  ...
  describe('debounce and serialization', () => {
    ...
    it('never runs passes concurrently', async () => {
      ...
      const controller = createController({ reload: runPass })
  ...
  describe('events during a reload', () => {
    it('runs exactly one following pass and settles idle', async () => {
      ...
      const controller = createController({ reload: pass.fn })
  ...
  describe('failures', () => {
    ...
    it('runs the following pass after a failed pass when events arrived during it', async () => {
      const pass = deferredReload()
      const onReloadError = vi.fn()
      const controller = createController({ reload: pass.fn, diagnostics: { onReloadError } })
    ...
    it('reports watcher errors without terminating', async () => {
      const onWatcherError = vi.fn()
      const controller = createController({ diagnostics: { onWatcherError } })
